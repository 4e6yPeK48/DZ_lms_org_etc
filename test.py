"""
class Card (масть, название (2-10, валет, дама, король, туз)):
    def __init__(self, масть, название):
        self.масть = масть
        self.название = название
пики крести бубны черви
♣ ♦ ♥ ♠

функция по номеру от 0 до 51 (включительно) определить карту
"""
...
"""  
правила игры:
Дилер раздаёт по две карты каждому игроку и себе, причём одна карта дилера остаётся закрытой.
Игроки по очереди принимают решение: взять карту ("hit") или остановиться ("stand"). 
Их цель — набрать 21 очко или как можно ближе к этому числу, не превышая его.
Когда все игроки завершили свои ходы, дилер открывает свою закрытую карту и тянет карты, пока не наберет минимум 17 очков.
Побеждает тот, у кого сумма очков ближе к 21, но не больше.
"""
...
class BlackJackGame:
    ...
    def play(self):
        ...
        while True:
            ...
            take = input('Взять или остановиться: ')
            match take.lower():
                case 'взять' | 'take' | 'hit':
                    self.hit()
